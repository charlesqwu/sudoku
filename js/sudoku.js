...
Sudoku.prototype.resizeWindow = function(){
    ...
    if (screen_wr > screen.h) {
        $('#'+ this.id +' .sudoku_board').css('width', (screen.h - b_pos.top - s_dim.h - 14)  * zoom_factor);
        $('#'+ this.id +' .board_console').css('width', (b_dim.h/2) * zoom_factor);
    } else {
    ...
    $('#'+ this.id +' .board_console').css({'left': c_pos_new.left - 100, 'top': c_pos_new.top});
